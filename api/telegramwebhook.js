...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (msg && typeof msg.text === 'string' && (msg.text.startsWith('/addstock') || msg.text.startsWith('/stock')) && !isAdmin) {
      // Не совпало с админом — вместо тишины объясняем, почему (это временная
      // диагностика на время настройки, потом можно убрать).
      await callTelegram('sendMessage', {
        chat_id: msg.chat.id,
        text: `⛔ Не похоже, что ты админ.\nТвой id: ${msg.from ? msg.from.id : '?'}\nОжидается (ADMIN_TELEGRAM_ID): ${ADMIN_TELEGRAM_ID ? ADMIN_TELEGRAM_ID : 'НЕ ЗАДАН на сервере'}`,
      });
    } else if (isAdmin && typeof msg.text === 'string' && (msg.text.startsWith('/addstock') || msg.text.startsWith('/stock'))) {

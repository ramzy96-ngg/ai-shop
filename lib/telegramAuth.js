...
function blankUser(id) {
  return {
    ...
    totalSpentStars: 0,
    totalSpentRub: 0,
    orders: [], // { title, amount, currency: 'XTR'|'RUB', items, date, status }
    createdAt: Date.now(),
  };
}
...
// оформлен через корзину без реальной оплаты (демо-режим).
async function recordOrder(telegramId, { title, amount, currency, items, deliveredCodes }) {
  ...
  u.orders.unshift({
    ...
    currency,
    items: items || null,
    delivered: !!(deliveredCodes && deliveredCodes.length),
    date: Date.now(),
    status: currency === 'XTR' ? 'paid' : 'placed',
  });
  await kv.set(userKey(id), u);
  return u;
}
